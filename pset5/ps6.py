...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        shiftLowChar = lowChar[shift:] + lowChar[:shift]
        shiftUpChar = upChar[shift:] + upChar[:shift]
        keys = lowChar + upChar
        values = shiftLowChar + shiftUpChar

        self.shiftDict = dict(zip(keys, values))
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(26):
            cipherMessage = self.apply_shift(s)
            decrypt = cipherMessage.split(" ")
            count = 0
            for i in decrypt:
        ...
        decipheredText = "".join(self.apply_shift(shiftVal))
        
        return shiftVal, decipheredText
